...
var d3Resume = function (_config) {
    ...
    var loadData = function (error, data) {
        ...
        normalize(data.society);

        x.domain([
            d3.min([
                d3.min(data.experience, function (d) {
                    return d.from;
                }),
                d3.min(data.study, function (d) {
                    return d.from;
                }),
                d3.min(data.society, function (d) {
                    return d.from;
                })
            ])
            ,
            d3.max([
                d3.max(data.experience, function (d) {
                    return d.pto;
                }),
                d3.max(data.study, function (d) {
                    return d.pto;
                }),
                d3.max(data.society, function (d) {
                    return d.pto;
                })
            ])
        ]);
        ...
        calculateDiameter(data.society);
        ...
        loadItems(svg, graphContainer, data.experience, "experience", -1, config.height / 8, getItemFillCollor);
        //loadItems(svg, graphContainer, data.society, "society", 0.5, config.height / 8, getItemFillCollor);
        loadItems(svg, graphContainer, data.study, "study", 1, config.height / 8, getItemFillCollor);
    }
    ...
    var loadItems = function (svg, graphContainer, data, className, position, infoTopPosition, getItemFillCollor) {
        ...
        var gInfo = svg
            ...
            .attr('class', function (d) {
                return className + d.id
            })
            .classed('info', true)
            .classed('default', function (d) {
                return d.default_item;
            })
        ...
        addItemDetail(gInfo, "18px", "translate(" + [0, 0] + ")", "normal", function (d) {
            return d.type;
        });
        addItemDetail(gInfo, "18px", "translate(" + [0, 25] + ")", "normal", function (d) {
            return d.title;
        });
        addItemDetail(gInfo, "23px", "translate(" + [0, 50] + ")", "normal", function (d) {
        ...
        addItemDetail(descriptionWrapper, "14px", function (d) {
    ...
    function getItemFillCollor(item) {
        return colorScale(item.id);
    }
